...
\only<2->{%
  ...
  \item what do we currently use the trait for?
  \end{itemize}
}
\end{frame}
...
\begin{frame}
  ...
  \item we know how to tear down \texttt{FreeBool}, but not yet how to interpret \texttt{Search} values
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \resizebox{7cm}{!}{\begin{tikzpicture}[sibling distance=10em,
  every node/.style = {shape=rectangle, draw, align=center, color=beamer@codeblue}]]
  \node[color=beamer@centricgreen, rounded corners] {And}
    child { node {Term(``FP'')} }
    child { node[color=beamer@centricgreen, rounded corners] {And}
...
\begin{frame}
  ...
  \begin{itemize}
  \item problem: you might have only partial information
  \item e.g.\ full text of indexed website slow to load
...
\begin{frame}[fragile]
  ...
  \begin{center}
  \inputminted[fontsize=\footnotesize]{scala}{snippets/functionalconf-meta-site.scala}
          ...
          child { node[color=beamer@centricgreen, rounded corners] {And}
            ...
            child { node[color=beamer@centricgreen, rounded corners]{And}
              child { node[color=beamer@centricgreen, rounded corners] {Not}
                child { node[color=beamer@centricgreen, rounded corners] {Or}
                  child { node[color=red!90] {False} }
                  child { node[dashed] {InText(``spring'')} }
                }
              }
              child { node[color=red!90]{True}}
            }
...
\begin{frame}[fragile]
  \frametitle{Partial Evaluation}
    \begin{center}
      ...
      \resizebox{3cm}{!}{
        \begin{tikzpicture}[sibling distance=10em, every node/.style = {shape=rectangle, draw, align=center, color=beamer@codeblue}]]
          \node[color=beamer@centricgreen, rounded corners] {Not}
          child { node[dashed] {InText(``spring'')} }
          ;
        \end{tikzpicture}
      }
